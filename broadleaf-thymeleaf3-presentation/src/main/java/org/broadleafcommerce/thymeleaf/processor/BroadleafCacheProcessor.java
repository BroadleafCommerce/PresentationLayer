...
import org.broadleafcommerce.common.web.cache.service.TemplateCacheKeyResolverService;
...
public class BroadleafCacheProcessor extends AbstractAttributeModelProcessor {
    ...
    protected void doProcess(ITemplateContext iContext, IModel model, AttributeName attributeName, String attributeValue, IElementModelStructureHandler structureHandler) {
        ...
        BroadleafThymeleafContext context = new BroadleafThymeleafContextImpl(iContext, structureHandler);
        if (shouldCache(attributeValue, context)) {
            ...
            if (cacheElement != null) {
                replaceTagWithCache(cacheElement.getObjectValue().toString(), model, iContext);
            } else {
    ...
    protected boolean shouldCache(String attributeValue, BroadleafThymeleafContext context) {
        if (isCachingEnabled()) {
     ...
     * @return
     */
    protected Element checkCacheForElement(Map<String, String> tagAttributes, String cacheKey) {
        
        if (!StringUtils.isEmpty(cacheKey)) {
            Element cacheElement = getCache().get(cacheKey);
    ...
    protected void addToCache(String cacheKey, IModel model, ITemplateContext context, Map<String, String> tagAttributes) {
        ...
        Element element = new Element(cacheKey, templateWriter.toString());
